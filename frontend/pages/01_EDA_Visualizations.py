...
def load_data():
    ...
    try:
        ...
        # CRITICAL FIX: Ensure claim_date is created properly
        ...
        # Simplified region to states mapping
        region_to_states = {
        ...
        # DEBUG: Print original dataframe info
        print("Original DF columns:", df.columns.tolist())
        print("Original DF shape:", df.shape)
        print("claim_date in original df:", 'claim_date' in df.columns)
...
# --- 4. Data Filtering and Preprocessing (COMPLETELY FIXED) ---
# Handle empty selections (prevents filtering out all data)
if not sel_gender:
...
# Create boolean mask for filtering with proper NaN handling
mask = (
...
# FIXED: Handle missing data without chained assignment warnings
df_filtered = df_filtered.copy()  # Ensure it's a true copy
...
# FIXED: Safe claim_date processing with error handling
if 'claim_date' in df_filtered.columns:
    ...
    # FIXED: add_trace instead of add_add_trace
    ...
    st.error("claim_date column is missing from the dataset.")
...
# FIXED: Added missing tab labels
...
with tab_fraud_analysis:
    ...
    # FIXED: Added color parameter and valid color scale
